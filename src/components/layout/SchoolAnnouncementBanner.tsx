...
export function SchoolAnnouncementBanner() {
  const { data: announcements = [] } = useSchoolAnnouncements();
          ...
          <div key={a.id} className={`flex items-start gap-3 p-3 rounded-lg border ${bgClass}`}>
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium">
                {a.title}
                {a.priority === 'high' && (
                  <span className="ml-2 text-[10px] font-semibold uppercase tracking-wide opacity-70">Important</span>
                )}
              </p>
